...
#define THINGSPEAK_API_WRITEKEY "94R8F61EKX9BXNGQ"
#define version 100
...
*/
    ...
    if (action == 3 && !setmode && cardSupported) {
        ...
        if (sernum == 0xd01bf755 || sernum == 0xEB338645 || sernum == 0x6EF10423) {
            ...
            // Writing new value block A
            // Scrivo i valori per il settore A
            byte value1Block[] = { 'H', 'y', 'p', 'a', 't', 'i', 'a', ' ', 'T', 'a', 'm', ' ',  '2','0','0','2',   valueBlockA,~valueBlockA,valueBlockA,~valueBlockA };
            status = mfrc522.MIFARE_Write(valueBlockA, value1Block, 16);
            if (status != MFRC522::STATUS_OK) {
                Serial.print("MIFARE_Write() failed: ");
                Serial.println(mfrc522.GetStatusCodeName(status));
        ...
        if (strncmp("Hypatia Tam 2002", (char *)buffer, 16) == 0){
            
            // sel a scrittura è uguale alla lettura allora e stato un successo !!
            display_string_5x7c(3,1,"Welcome Hypatia!");

        }else{
            
            //  scrittura Fallita
            display_string_5x7c(4,1,"I don't know you ... ");
            display_string_5x7c(5,1,":-( ");
        ...
        // ENSURE mode
        // Ensure proper use by presenting the appropriate card first before any action
        // Just to annoy :)
        sprintf(msg, "(%0lx)", sernum);
        display_string_5x7c(3,1,"");
        display_string_5x7c(4,1,"Use READ/WRITE card!");
        display_string_5x7c(5,1,msg);
        stimer = 0; tinc = 1;
    }
    // risponde successo
    //Serial.println(mfrc522.GetStatusCodeName(status));
