...
public:
	...
	Si4432(uint8_t nselPin, uint8_t sdnPin, uint8_t InterruptPin = 0); // when a InterruptPin is given, interrupts are checked with this pin - rather than SPI polling
	...
	byte sendPacket(uint8_t length, const byte* data, bool waitResponse = false, uint32_t ackTimeout = 100,
	...
	byte isPacketReceived(); // check for the packet received flags
...
protected:
	...
	uint8_t _nselPin, _sdnPin, _intPin;
